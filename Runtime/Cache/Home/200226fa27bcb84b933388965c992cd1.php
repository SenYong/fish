...
<body>
  ...
  <article>
    <input type="hidden" value="<?php echo ($id); ?>" id="pid">
    <div class="l_box f_l">
	   <div class="topnews">
  	    <h2>
      	   <span>
      	     <?php if(is_array($cat)): $i = 0; $__LIST__ = $cat;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="<?php echo U('/cat-'.$vo['c_id']);?>" <?php if($vo['c_id'] == $id): ?>class='active'<?php endif; ?> ><?php echo ($vo["c_name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
      	   </span>	 
    	     <a href="<?php echo U('/article');?>">技术心得</a>
  	    </h2>
        <div id="list">
            <?php if(is_array($catList)): $i = 0; $__LIST__ = $catList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="blogs">
        		      <figure>
        			      <img src="<?php echo ($vo["a_img"]); ?>">
        			   </figure>
        			   <ul>
        			      <h3><a href="<?php echo U('/artinfo-'.$vo['a_id']);?>"><?php echo ($vo["a_name"]); ?></a></h3>
        				    <p><?php echo ($vo["a_desc"]); ?></p>
        			      <p class="autor">
          				    <span class="lm f_l">
          					    <a href="/"><?php echo ($vo["c_name"]); ?></a>
          					 </span>
        					   <span class="dtime f_l"><?php echo (date("Y-m-d",$vo["a_time"])); ?></span>
          					 <span class="viewnum f_r">
          					    浏览（<a href="<?php echo U('/artinfo-'.$vo['a_id']);?>"><?php echo ($vo["a_hit"]); ?></a>）
                     </span>
          					 <span class="pingl f_r">	
          					    评论（<a href="<?php echo U('/artinfo-'.$vo['a_id']);?>"><?php echo ($vo["a_num"]); ?></a>）
                     </span>
        				    </p>
        			   </ul>
            ...
            success: function(data){
              if(data.length){
                $.each(data,function(index,item){
                   var a_id = item.a_id
                   var str = "<div class=\"blogs\"><figure><img src=\""+item.a_img+"\"></figure><ul><h3><a href=\"<?php echo U('/artinfo-'.'a_id');?>\">"+item.a_name+"</a></h3><p>"+item.a_desc+"</p><p class=\"autor\"><span class=\"lm f_l\"><a href=\"\">"+item.c_name+"</a></span><span class=\"dtime f_l\">"+date(item.a_time)+"</span><span class=\"viewnum f_r\">浏览（<a href=\"<?php echo U('/artinfo-'.'a_id');?>\">"+item.a_hit+"</a>）</span><span class=\"pingl f_r\">评论（<a href=\"<?php echo U('/artinfo-'.'a_id');?>\">"+item.a_num+"</a>）</span></p></ul></figure></div>";
