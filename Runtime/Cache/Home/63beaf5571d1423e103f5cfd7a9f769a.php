...
<body>
  ...
  <article>
    <div class="l_box f_l">
	   <div class="topnews">
  	    <h2>
      	   <a href="<?php echo U('/say');?>">畅所欲言</a>
  	    </h2>
        <div id="list">
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="mood">
                <span class="tutime"><?php echo (date("Y-m-d H:i:s",$vo["s_time"])); ?></span>
                <p class="p1"><a href="<?php echo U('/sayinfo-'.$vo['s_id']);?>" class="cont"><?php echo ($vo["s_content"]); ?></a></p>
                <p class="p2"><a class="span1" href="">浏览(<?php echo ($vo["s_hit"]); ?>)</a><a class="span2" href="">评论(<?php echo ($vo["s_num"]); ?>)</a></p>
            ...
            success: function(data){
              if(data.length){
                $.each(data,function(index,item){
                   var l_id = item.l_id
                   var str = "<div class=\"mood\"><span class=\"tutime\">"+date(item.s_time)+"</span><p class=\"p1\"><a href=\"<?php echo U('/sayinfo-'.'s_id');?>\" class=\"cont\">"+item.s_content+"</a></p><p class=\"p2\"><a class=\"span1\">浏览("+item.s_hit+")</a><a class=\"span2\">评论("+item.s_num+")</a></p></div>"
