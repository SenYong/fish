      ...
      <div class="row">
        <div class="add-article-form">
          <div class="col-md-9">
              ...
              <input type="hidden" value="<?php echo (session('uname')); ?>" id="ac_rename">
              <input type="hidden" value="<?php echo (session('uimg')); ?>" id="ac_reimg">
              <input type="hidden" value="<?php echo ($info["ac_id"]); ?>" id="ac_id">
              <h2 class="add-article-box-title"><span>用户名</span></h2>
              <div class="form-group">
                <div id="article-title" class="form-control"><?php echo ($info["ac_name"]); ?></div>
              </div>
              <div class="add-article-box">
                <h2 class="add-article-box-title"><span>评论内容</span></h2>
                <div class="add-article-box-content">
                  <div class="form-control" id="desc"><?php echo (reFace($info["ac_content"])); ?></div>
                </div>
              </div>
              <h2 class="add-article-box-title"><span>评论时间</span></h2>
              <div class="form-group"> 
                 <div id="article-title" class="form-control"><?php echo (getTime($info["ac_time"])); ?></div>
              </div>
              <h2 class="add-article-box-title"><span>回复内容</span></h2>
              <div class="form sayForm" id="comment">
                  <div class="warp_text">
                      <textarea id="content-text" placeholder="回复些什么呢..."><?php echo (htmlspecialchars_decode($info["ac_recontent"])); ?></textarea>
                  </div>
          ...
          <div class="col-md-3">
            ...
            <div class="add-article-box">
              ...
              <div class="add-article-box-content">
              	<p>
                   <label>状态：</label>
                   <?php if(empty($info['ac_retime'])): ?><span class="article-status-display">未回复</span>
                   <?php else: ?>
              ...
              <div class="add-article-box-footer">    
                  <button class="btn" type="button"  id="cancel" style="margin-left: 20px"><a href="/Admin/Article/artContentList">取消</a></button>
                  <button class="btn btn-primary" type="submit" name="submit" id="reArt">回复</button>
              </div>
            </div>
          </div>
        </div>
...
$('#reArt').click(function(){
   var ac_id = $('#ac_id').val();
   var ac_recontent = $('#content-text').val();
   var ac_rename = $('#ac_rename').val();
   var ac_reimg = $('#ac_reimg').val();
   if(ac_recontent == ''){
     error('回复内容不能为空');
   }
   if(ac_reimg == ''){
      ac_reimg = '/Public/Home/img/default.png';
   }
   $('#reArt').attr('disabled',true);
   var data = {'ac_id':ac_id, 'ac_rename':ac_rename, 'ac_reimg':ac_reimg, 'ac_recontent':ac_recontent}
   console.log(data)
   $.ajax({
     url: '/Admin/Article/artContentModify',
     type: 'post',
     async: true,
     data: {'ac_id':ac_id, 'ac_rename':ac_rename, 'ac_reimg':ac_reimg, 'ac_recontent':ac_recontent},
     success: function (data) {
         console.log(data);
         if(data.error == 0){
            success('',data.msg,'/Admin/Article/artContentList');
         }else{
            error(data.msg);
            $('#reArt').removeAttr('disabled');
         }
     },
     error: function(data){
         error(data.msg);
