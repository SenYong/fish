...
<div class="row">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <form action="/Article/checkAll" method="post" >
        <h1 class="page-header">留言板列表 <span class="badge"><?php echo ($num); ?></span></h1>
          ...
          <table class="table table-striped table-hover">
            ...
            <tbody>
              <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                    <td><?php echo ($k); ?></td>
                    <td class="article-title"><?php echo ($vo["b_name"]); ?></td>              
                    <td class="hidden-sm"><?php echo (reFace($vo["b_content"])); ?></td>
                    <td><?php echo (date("Y-m-d H:i:s",$vo["b_time"])); ?></td>
                    <?php if(empty($vo['b_retime'])): ?><td class="not">否</td>
                    <?php else: ?>
                       <td>是</td><?php endif; ?>
                    <td>
                       <a href="<?php echo U('/Admin/Board/boardContentEdit',array('id'=>$vo['b_id']));?>">回复</a> 
